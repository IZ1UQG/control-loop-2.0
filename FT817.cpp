...
void FT817::rigComError(char * string) {
  ...
  // invece esegue il ciclo getFreqMode finché non otteniamo dati validi
}
  ...
  if (freqOut == freq) {
    ...
    */
    ...
    if (DEBUG) {
     ...
     // rigComError("setfreqtest fallito");
    return false;
  }
}
...
unsigned long FT817::getFreqMode() {
  rigSoftSerial.flush();            // non sono sicuro del motivo per cui devo farlo.
    
  boolean readValid = true;         // controlla i dati dopo setFreq
  fourBytePreamble();
  ...
  if (elapsed >= 2000) {
    if (DEBUG) {
      Serial.println("timeout on getfreqMode");
    }
    readValid = false;
  }
  else {
